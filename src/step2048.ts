...
interface IAnimationLoop {
    run(animations: Array<CombinedAnimator>): void;
    cancel(): void;
  }
  

async function step2048(sumScore: Function, grid: Array<Array<IElement>>, dir: string, animLoop: IAnimationLoop) {
    const mergeBorders:Array<number> = [0, 0, 0, 0];
...
function linearMotion(si:number, sj:number, di:number, dj:number, value:number) {
  ...
  function step2048iteration(sumScore:Function, grid: Array<Array<IElement>>, dir:string, mergeBorders:Array<number>) {
    const n: number = grid.length;
